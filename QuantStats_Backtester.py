...
DEFAULT_STRATEGY = '''\
# ══════════════════════════════════════════════════════════════════════════
# Strategy function: return a signal Series  OR  a dict.
#
#   def strategy(data: pd.DataFrame) -> pd.Series | dict
#
# INPUT  data  (UTC-aware DatetimeIndex):
#   open, high, low, close, volume, vwap
#
# OUTPUT — OPTION A (signal only):
#   pd.Series:  +1 = Long   0 = Flat   -1 = Short
#
# OUTPUT — OPTION B (signal + chart indicators):
#   {
#     "signal":   pd.Series,
#     "overlays": {"Name": pd.Series, ...},   # drawn on price chart
#     "panels":   {"Name": pd.Series, ...},   # separate sub-charts
#   }
#
...
def build_interactive_chart(data: pd.DataFrame,
                            trade_log: pd.DataFrame,
                            indicators: dict = None,
                            ticker: str = "",
                            strategy_name: str = "") -> str:
    """
    Build a TradingView-style interactive HTML candlestick chart.

    Contents:
      Row 1 — OHLC candlesticks + overlay indicators + trade markers
      Row 2 — Volume bars (coloured by direction)
      Row 3+ — One row per panel indicator (RSI, MACD, etc.)
    ...
    panels   = indicators.get("panels",   {}) if indicators else {}

    n_panels = len(panels)
    n_rows   = 2 + n_panels

    if n_panels:
        heights = [0.55, 0.15] + [round(0.30 / n_panels, 3)] * n_panels
    else:
        heights = [0.70, 0.30]

    subplot_titles = [f"{ticker.upper()}  —  {strategy_name}", "Volume"]
    ...
    _COLORS = ["#2962FF", "#FF6D00", "#AB47BC", "#00BCD4",
               "#FFEB3B", "#E91E63", "#4CAF50", "#FF5722"]
    for idx, (name, series) in enumerate(overlays.items()):
        s = series.reindex(data.index)
    ...
    # ── Panel indicators ─────────────────────────────────────────────────
    for pidx, (name, series) in enumerate(panels.items()):
        s = series.reindex(data.index)
        fig.add_trace(go.Scatter(
            x=s.index, y=s.values, name=name, mode="lines",
            line=dict(color=_COLORS[pidx % len(_COLORS)], width=1.5),
        ), row=3 + pidx, col=1)

    # ── Trade markers ────────────────────────────────────────────────────
    if not trade_log.empty:
        long_e  = trade_log[trade_log["direction"] == "Long"]
        short_e = trade_log[trade_log["direction"] == "Short"]
        ...
        if not long_e.empty:
            fig.add_trace(go.Scatter(
                ...
                text=[f"Long #{int(r['trade_num'])}<br>"
                      f"Entry ${r['entry_price']:.4f}"
                      for _, r in long_e.iterrows()],
                hoverinfo="text",
            ), row=1, col=1)
        ...
        if not short_e.empty:
            fig.add_trace(go.Scatter(
                ...
                text=[f"Short #{int(r['trade_num'])}<br>"
                      f"Entry ${r['entry_price']:.4f}"
                      for _, r in short_e.iterrows()],
                hoverinfo="text",
            ), row=1, col=1)

        # Exits — coloured by win / loss
        wins   = trade_log[trade_log["pnl_pct"] > 0]
        losses = trade_log[trade_log["pnl_pct"] <= 0]
        ...
        if not wins.empty:
            fig.add_trace(go.Scatter(
                ...
                text=[f"Exit #{int(r['trade_num'])}<br>"
                      f"${r['exit_price']:.4f}<br>"
                      f"P&L {r['pnl_pct']:+.3f}%"
    ...
    fig.update_layout(
        ...
        title=dict(text=f"📈  {strategy_name}  —  {ticker.upper()}",
                   font=dict(size=18)),
    ...
    html_path = os.path.join(
        tempfile.gettempdir(),
        f"chart_{safe}_{datetime.now():%Y%m%d_%H%M%S}.html")
...
def run_backtest(data: pd.DataFrame,
    ...
    try:
        raw_result = ns["strategy"](data.copy())
    except Exception:
        raise RuntimeError(
            f"Strategy raised an exception at runtime:\n{traceback.format_exc()}")

    # Support dict return: {"signal": Series, "overlays": {...}, "panels": {...}}
    indicators = None
    if isinstance(raw_result, dict):
        if "signal" not in raw_result:
            raise ValueError("Strategy dict must contain a 'signal' key.")
        raw_sigs = raw_result["signal"]
        indicators = {
            "overlays": {k: v for k, v in raw_result.get("overlays", {}).items()},
            "panels":   {k: v for k, v in raw_result.get("panels",   {}).items()},
        }
    elif isinstance(raw_result, pd.Series):
        raw_sigs = raw_result
    else:
        raise ValueError("`strategy()` must return a pd.Series or a dict.")
    ...
    # ── 5b. Interactive chart ─────────────────────────────────────────────
    chart_html = build_interactive_chart(
        data=data, trade_log=trade_log, indicators=indicators,
...
class App(tk.Tk):
    ...
    def _sidebar(self, parent):
        ...
        self._chart_open_btn = ttk.Button(
            bf, text="📈  Open Interactive Chart",
            style="Open.TButton", command=self._open_chart_html, state="disabled")
        self._chart_open_btn.pack(fill="x", pady=(4, 0))
    ...
    def _bt_thread(self):
        try:
            ...
            self._chart_html = result["chart_html"]

            # ── Log summary ───────────────────────────────────────────────
            self._log_w(f"Tearsheet saved: {result['html_path']}", "ok")
            self._log_w(f"Interactive chart saved: {result['chart_html']}", "ok")
    ...
    def _show_bt_results(self, result: dict):
        ...
        self._chart_open_btn.configure(state="normal")
        self._nb.select(1)
        self._status.set(
    ...
    def _open_chart_html(self):
        if self._chart_html and os.path.exists(self._chart_html):
            webbrowser.open(f"file://{self._chart_html}")
        else:
            messagebox.showwarning("No Chart",
                                   "Run a backtest first to generate the chart.")
